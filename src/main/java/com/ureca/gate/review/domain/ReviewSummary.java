...
public class ReviewSummary {
    ...
    // 생성자
    public ReviewSummary(Place place, ReviewSummaryType type, String comment) {
        this.id = null; // ID는 데이터베이스에서 자동 생성
        this.place = place;
        this.type = type;
    ...
    public static List<ReviewSummary> createFromParsedSummary(Place place, Map<ReviewSummaryType, String> parsedSummary) {
        return parsedSummary.entrySet().stream()
                .map(entry -> new ReviewSummary(place,entry.getKey(),entry.getValue()))
