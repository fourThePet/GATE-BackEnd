...
public class GptServiceImpl implements GptService {
    ...
    @Override
    public String getRegion(String query) {
        return chatClient.prompt()
                .system("""
                 1. 너는 사용자가 입력한 텍스트에서 **지역명(시/군/구 + 동/리)**과 나머지 검색어를 구분하고 구조화하는 전문가이다. \s
                 2. 사용자가 제공한 입력에서 다음 네 가지를 추출해야 한다: \s
                   - **시**: 광역시나 특별시, 도 단위의 시를 나타낸다. \s
                       예: "서울", "부산", "울산" \s
                   - **군/구**: 시 하위의 군 또는 구 단위를 나타낸다. \s
                        예: "성동구", "중구" \s
                   - **동/리**: 군/구 하위의 동 또는 리 단위를 나타낸다. \s
                        예: "성수동", "우정동" \s
                   - **검색어**: 지역명을 제외한 나머지 입력 내용으로 사용자의 검색 목적을 나타낸다. \s
                 3. **출력 형식**은 반드시 아래와 같아야 한다: \s
                   - 시: [시 단위]
                     군/구: [군/구 단위]
                     동/리: [동/리 단위]
                     검색어: [나머지 검색어]
                4. **규칙**: \s
                   - 입력된 텍스트에서 지역명은 반드시 시 → 군/구 → 동/리 순으로 나누어 표시해야 한다. \s
                   - **같은 이름의 군/구가 복수 존재할 경우, 제일 상위의 시(서울, 부산 등)를 기준으로 반환하라.** \s
                      예: \s
                      - "중구 맛집" → `시: 서울, 군/구: 중구, 동/리: , 검색어: 맛집` \s
                      - "울산 중구 맛집" → `시: 울산, 군/구: 중구, 동/리: , 검색어: 맛집` \s
                   - 지역명이 입력되지 않은 경우, 해당 필드는 빈 값으로 두어라. \s
                      예: "맛집 추천" → `시: , 군/구: , 동/리: , 검색어: 맛집 추천` \s
                5. **예시 입력과 출력**: \s
                          - 입력: "성수 카페" \s
                            출력: \s
                            시: 서울 \s
